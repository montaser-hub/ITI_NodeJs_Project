  ...
  if (cart) {
    for (const newItem of itemDetails) {
      ...
      if (existingItem) {
        ...
        existingItem.priceAtTime = newItem.priceAtTime;
        existingItem.subTotal =
          existingItem.quantity * existingItem.priceAtTime;
      } else {
    ...
    cart = await cartModel.create({
      titleCart: titleCart || "Default Cart",
      userId: req.user._id,
      items: itemDetails,
      totalPrice: itemDetails.reduce((sum, item) => sum + item.subTotal, 0),
    });
  }
...
export const removeCartItem = catchError(async (req, res, next) => {
  ...
  if (cart.items.length === 0) {
    ...
    return res.status(200).json({
      message:
        "Product removed successfully, cart deleted because it was empty",
    });
  }
  await cart.save();
  res.status(200).json({
